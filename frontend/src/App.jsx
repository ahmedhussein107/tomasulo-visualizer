...
function App() {
	...
	useEffect(() => {
		...
		const fetchSizes = async () => {
			try {
				...
				setCacheSize(Number(splittedLine[1].trim()) * Number(splittedLine[0].trim()));
	...
	const destructeStationId = (stationId) => {
		...
		let index = Number(stationId.slice(2));
		...
		const storeInCache = (address, value, isDoubleWord) => {
			...
			if (index !== -1) {
				...
				if (isDoubleWord) {
					...
					view.cache[index].block[offset] = Number(valueBigInt & 0xffffffffn); // Upper 32 bits
				}
			} else {
